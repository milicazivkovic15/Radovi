...
public class Dame extends Igra{
			...
			if (igrac.getKarta(i).getZnak()==znak){
				...
				while (igrac.getKarta(i)!=null && igrac.getKarta(i).getZnak()==znak ){
					if (igrac.getKarta(i).getBroj()<max && igrac.getKarta(i).getBroj()!=1){	
						if (max==15 && igrac.getKarta(i).getBroj()==14){
							if (igrac.getKarta(i+1)!=null && igrac.getKarta(i+1).getBroj()==13)
				...
				if (brKarataNaTalonu==3 && igrac.getKarta(j).getBroj()!=13)
					return igrac.getKarta(j);//ako vec nema da se podvuce da uzme najvecom
				
				if (igrac.getKarta(i-1).getBroj()==13)
					if (i-2>-1)
						if(igrac.getKarta(i-2)!=null && igrac.getKarta(i-2).getZnak()==znak)
	...
	public boolean preporucujemIgru(Igrac igrac) {
		for (int i = 0; i < igrac.getBrIgara(); i++) {
			if (igrac.getIgre(i) instanceof Dame && igrac.getIgre(i+1)==null)
				return true;
		}
		int br=-1;
		Znak z=igrac.getKarta(0).getZnak();
		boolean f=true;
		for (int j = 0; j < igrac.getBrKarata(); j++) {
			if (igrac.getKarta(j).getZnak()==z){
				if (igrac.getKarta(j).getBroj()==13){
					br=0;
				}
				else if (igrac.getKarta(j).getBroj()<13 && igrac.getKarta(j).getBroj()!=1){
					if (br>-1)
						br++;
					f=true;
				...
				if ((br<2 && br!=-1) || f==false){
					return false;
				}
				br=-1;
				z=igrac.getKarta(j).getZnak();
				j--;
				f=true;
			}
		}
